...
export function normalizeSeerData(input) {
  const result = {
    ...
    classifierScores: {}, // classifier id -> { id, name, score, category, tools }
  ...
  for (const [rawKey, rawVal, kind] of entries) {
    ...
    if (cls) {
      ...
      classifiersUsed++;
      result.classifierScores[cls.id] = {
        id: cls.id, name: cls.name, score: entry.score, category: cls.category, tools: cls.tools,
      };
