...
import { TOOLS } from './seer-taxonomy.js';

const MODEL = 'claude-sonnet-4-5-20250929';
const TOOL_NAME = Object.fromEntries(TOOLS.map((t) => [t.id, t.name]));
const CLASSIFIER_DRAG_THRESHOLD = 60; // scores below this count as "dragging"
...
function heuristicAnalysis(business, category, queries) {
  ...
  const evidence = {
    identity_entity: has('brand', 'established', 'years', 'family', 'specializ') || !!business.website,
    local_spatial: has('neighborhood', 'service area', 'local', 'city', 'county', business.location.toLowerCase()),
    trust_credentials: has('licens', 'insur', 'certif', 'bonded', 'warranty', 'nate', 'epa', 'bbb', 'osha'),
    content_utility: has('blog', 'faq', 'guide', 'cost', 'pricing', 'case study', 'content', 'service page') || !!business.website,
    linguistic_authenticity: has('blog', 'guide', 'article', 'content'),
    visual_intelligence: has('photo', 'team', 'gallery', 'image', 'before/after', 'before and after'),
    external_reputation: has('review', 'star', 'rating', 'yelp', 'google', 'press', 'featured', 'best of', 'award'),
    link_authority: has('backlink', 'link', 'directory', 'featured', 'press', 'chamber', 'association'),
    technical_infra: has('schema', 'google business', 'gbp', 'structured', 'sitemap') || !!business.website,
    integrity_risk: !has('template', 'thin', 'duplicate', 'doorway', 'fake'),
    behavioral_validation: has('24/7', 'emergency', 'booking', 'online schedul', 'call now') || !!business.website,
    temporal_dynamics: has('updated', 'recent', 'active', 'post', 'blog', 'weekly', 'monthly'),
  };
  ...
  } in ${category.label} for ${business.location}. Across SEER's 12 classifier categories, the fastest path to becoming the go-to recommendation is to make its strongest real-world signals verifiable — resolving a clear local entity and letting reviews, credentials and content corroborate it.`;

  return { summary, signals, queries: qResults, roadmap };
}

const ROADMAP_TEMPLATES = {
  identity_entity: { priority: 'Foundational', title: 'Consolidate one coherent entity: single specialty, consistent identity, Knowledge Graph schema (sameAs, Organization)', effort: 'Medium', impact: 'High', why: 'Engines must be confident who you are before they will recommend you.' },
  local_spatial: { priority: 'Foundational', title: 'Build localized proof: city/neighborhood pages, geotagged project galleries and area-specific reviews', effort: 'Medium', impact: 'High', why: 'Home services is local — verified spatial grounding is what lets you rank past the city line.' },
  trust_credentials: { priority: 'Quick Win', title: 'Publish & cross-link license #, insurance, certifications and warranties to verification sources', effort: 'Low', impact: 'High', why: 'Verifiable credentials turn self-claims into trust the model will repeat for a risk-aware category.' },
  content_utility: { priority: 'Compounding', title: 'Publish first-party, high-information service content with real pricing and case studies', effort: 'Medium', impact: 'High', why: 'Content with genuine experience and information gain is the passage engines ground their answer in.' },
  linguistic_authenticity: { priority: 'Compounding', title: 'Rewrite templated copy into specific, human, regionally-grounded language', effort: 'Medium', impact: 'Medium', why: 'AI-detection classifiers dampen generic copy; authentic language reads as real expertise.' },
  visual_intelligence: { priority: 'Quick Win', title: 'Replace stock imagery with authentic team and project photos', effort: 'Low', impact: 'Medium', why: 'Computer-vision classifiers flag stock/AI images and fail the experience check.' },
  external_reputation: { priority: 'Quick Win', title: 'Drive review volume & velocity and align reviews to your claimed strengths', effort: 'Low', impact: 'High', why: 'Independent reputation that corroborates your claims is the strongest external proxy engines cite.' },
  link_authority: { priority: 'Foundational', title: 'Earn relevant, entity-validating backlinks from local and industry sources', effort: 'Medium', impact: 'Medium', why: 'Links from trusted, related entities validate your identity in the graph.' },
  technical_infra: { priority: 'Quick Win', title: 'Add LocalBusiness/Service schema and sync it with your Google Business Profile', effort: 'Low', impact: 'Medium', why: 'Machine-legible, crawlable pages make you easy to assemble into an AI answer.' },
  integrity_risk: { priority: 'Foundational', title: 'Remove suppression triggers: dedupe thin city pages and templated service pages', effort: 'Medium', impact: 'Medium', why: 'A single domain-level dampener can cap every other signal — removing it lifts the ceiling.' },
  behavioral_validation: { priority: 'Quick Win', title: 'Cut conversion friction: visible phone/CTA above the fold and a fast emergency path', effort: 'Low', impact: 'Medium', why: 'NavBoost-class systems reward satisfied users, especially on urgent-intent queries.' },
  temporal_dynamics: { priority: 'Quick Win', title: 'Establish freshness: regular posts, ongoing review flow and updated content', effort: 'Low', impact: 'Medium', why: 'Engines favor businesses that read as active and improving, not decaying.' },
};

function buildHeuristicRoadmap(signals) {
  const weakest = [...signals]
    .map((s) => ({ ...s, weight: SIGNAL_MAP[s.key].weight }))
    // biggest weighted opportunity first: low score × high weight
    .sort((a, b) => (100 - b.score) * b.weight - (100 - a.score) * a.weight);

  return weakest.slice(0, 6).map((s) => ({ signalKey: s.key, ...ROADMAP_TEMPLATES[s.key] }));
}
...
function assembleResult(business, queries, raw, mode, seer, baseMode) {
  ...
  const seerClassifiers = (seer && seer.classifierScores) || {};
  ...
  const signals = SIGNALS.map((def) => {
    ...
    // Specific SEER classifiers dragging this signal down (lowest first).
    const catClassifiers = Object.values(seerClassifiers).filter((c) => c.category === def.key);
    const weakClassifiers = catClassifiers
      .filter((c) => c.score < CLASSIFIER_DRAG_THRESHOLD)
      ...
      .map((c) => ({
        ...
        name: c.name,
        score: c.score,
        tools: (c.tools || []).map((t) => TOOL_NAME[t] || t),
      }));
    ...
    return {
      ...
      classifierCount: def.classifierCount,
      why: def.why,
      short: def.short,
      score,
      ...
      weakClassifiers,
      scoredClassifiers: catClassifiers.length,
    };
  });
  ...
  return {
    ...
    seer: {
      ...
      toolsUsed: seer ? seer.toolsUsed : [],
      classifiersUsed: seer ? seer.classifiersUsed : 0,
      categoriesDirect: seer ? seer.categoriesDirect : 0,
      competitorContext: seer ? seer.competitorContext : null,
