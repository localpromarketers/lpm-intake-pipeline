...
export async function runAuthorityScan(input) {
  ...
  return assembleResult(business, queries, raw, mode);
}
...
function assembleResult(business, queries, raw, mode) {
  ...
  const signals = SIGNALS.map((def) => {
    ...
    return {
      ...
      whatEnginesSee: got.whatEnginesSee || '',
      gap: got.gap || '',
    };
  });

  const authorityIndex = Math.round(
    signals.reduce((a, s) => a + s.score * s.weight, 0) / 100
  );
  ...
  const roadmap = (raw.roadmap || []).map((r) => ({
